...
<table cellpadding="10" cellspacing="10" border="0" style="border-collapse: collapse; margin: auto">
	...
	<?php 
	require_once('conn.php');//kết nối với mysql
	$sql = "SELECT user.TENUSER, film.TENFILM, binhluan.NOIDUNG,binhluan.TIME, user.IDUSER, binhluan.IDCOMMENT
			...
			INNER JOIN film ON film.IDFILM= binhluan.PHIMCOMMENT"; //lấy ra cac bình luận
			
    $result = $conn->query($sql);//thực hiện chạy câu truy vấn
    
	if ($result->num_rows > 0) {// kiêm tra xem có bản ghi không nếu ko có bản ghi nào thì sẽ không chạy vào trong if.
		while ($row = $result->fetch_assoc()) {// nhả dữ liệu về một mảng.
	?>
    <tr class="item">
        <td><?= $row['TENUSER'] ?></td>
		<td><?= $row['TENFILM'] ?></td>
...
<br>
	<a href="logout.php" class="btn btn-primary btn-lg btn-block">
        Đăng xuất
</a>
